...
## 边界
- 允许新增：`onChange` 坐标回写（`PATCH /api/model/positions`）、双通道 `renderScene` 重绘、SSE 事件驱动的进度 / 统计 UI
- 允许修改：前端调用 `POST /api/chat`（SSE）、`PATCH /api/model/positions`
...
## 前置
- M6a 渲染镜像层可用（S1）
- v2-24b / v2-25 需 M1 的 `POST /api/chat` + **内存版 session**（M1 交付），`PATCH /api/model/positions` 亦由 M1 的内存 session 提供（M5 补 Redis 持久化）
- v2-25 另需 **M3 Diagram Tools**（S3）
- v2-26 需 **M4 Agent 事件流**（S4）

## 步骤
1. v2-24b 人类拖拽 `onChange` → `syncPositions` → `PATCH /api/model/positions` 回写后端 model（**S3**）
2. v2-25 双通道：AI 聊天命令 → 后端 Diagram Tools（自研 MCP）→ 增量 `ops` / 全量 `spec` → `renderScene` 重绘（**S3**）
3. v2-26 SSE 过程可视化 + Token 统计 + 错误明细展示（**S4**）

## 验收口径
1. AI 改图与人类拖拽汇同一份后端 model，**互不覆盖**
2. 用户手绘 / 拖拽后刷新，坐标已回写后端（非仅前端内存）
3. 增量 `ops` 与全量 `spec` 都经 `renderScene` 正确落地
