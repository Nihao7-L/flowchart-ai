...
> 状态：完成（2026-09-12，v2-1 ~ v2-7 全部闭合）  
> 关联架构：`architecture.md` 2.4 包表、`coreFun.md` 能力量级  
> 覆盖任务：v2-1 ~ v2-7（全部完成，M0 工程底座闭合）  
> 归档：2026-09-14 由 `plans/masterPlan/` 移入 `state/completed/masterPlan/`（M0 全闭合后归档）。

## 元信息

- 模块编号：M0
- 状态：完成（2026-09-12）
- 创建：2026-09-11 + AI ｜ 完成：2026-09-12 + AI
...
5. v2-5 GitHub Actions **重建**并真跑 —— ✅ 已落地 09-12（原表述"修正"与事实不符：`reframing`/`main` 均无 `.github` 目录，旧配置只在历史提交 `ecbaa68` 里，实为重建；`ci.yml` 写两个并行 job、三步展开，并补 `run-verify.ps1` 前端 lint 使两边对齐；push 后 run #2 全绿，43s = 后端 39s + 前端 17s）
...
1. `powershell -File run-verify.ps1` 末行 `VERIFY PASS`、退出码 0 —— ✅ 09-12 已验证（14:30 真跑：`VERIFY PASS` + 退出码 0，当时前端按 `-SkipFrontend` 跳过）；⚠️ 09-12 给脚本补前端 `lint` 后，**脚本的完整端到端（含前端三步）待用户在本地跑一次确认**（`lint` 本身已在 CI 前端 job 验证为绿）
2. CI 每次 push 真跑且绿 —— ✅ 09-12 已验证（run #2：`reframing`@`67cacc9`，43s，后端 job 39s + 前端 job 17s，两 job step 级全 success；日志核对 `Tests run: 58` / `You have 0 violations.`）
...
- [x] v2-5 GitHub Actions（09-12 完成：原计划「修正」实为「**重建**」——`reframing`/`main` 两分支均无 `.github`，旧版只在历史提交 `ecbaa68` 里；新建 `ci.yml`（两个并行 job、三步展开）+ `run-verify.ps1` 补前端 lint；push 后 run #2 **全绿**（43s，`reframing`@`67cacc9`，后端 39s / 前端 17s，step 级全 success；日志核对 58 测试 + 0 违规）。另发现并闭合一处**首跑必红**的阻塞：`frontend/`、`checkstyle.xml`、`src/test` 等 36 个文件从未 `git add`，已随提交 `67cacc9` 入库。详见 `state/completed/v2-5-ci-github-actions.md`）
